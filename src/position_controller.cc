...
namespace gazebo
{
  ...
  class PositionPlugin : public ModelPlugin
  {
    ...
    public: virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
      ...
      if (_sdf->HasElement("joint_name"))
      {
        ...
        if (joint_name_ori.find(_model->GetName() + "::") == 0)
        {
          ...
          std::cerr << "We find the joint [" << joint_name_ori << "]\n";
        }
        else
        {
          joint_name =  _model->GetScopedName() + "::" + joint_name_ori;
          std::cerr << "We find the joint [" << joint_name << "]\n";
        }
      }
    ...
    public: void SetPosition(const double &_pos)
    {
      ...
      this->model->GetJointController()->SetPositionTarget(
          joint_name, _pos);
      // print the joint_pos via roslog
      ROS_INFO("\n The joint's position [%s] is [%f]\n", joint_name.c_str() ,_pos);
    }
